...
def create_kvasir_user():
    if local("sudo -u postgres psql postgres -tAc \"SELECT 1 FROM pg_roles WHERE rolname=\'kvasir\'\"", capture = True) == 1:
...
def create_kvasir_db():
    dbs = local("sudo -u postgres psql -l", capture = True)
    #if not local("sudo -u postgres psql -l | grep \"^ kvasir\"", capture = True):
    if not "kvasir" in dbs:
...
def make_symlinks():
    response = local("grep 'data_directory' /etc/postgresql/*/main/postgresql.conf", capture = True)
    data_directory = response.split("'")[1]
